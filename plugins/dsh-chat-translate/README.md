...
- **实时行整行扣留（按阅读顺序放行）**：当前 turn 里新追加的工具调用行（以及命令行等所有被翻译的行）在标题译文就绪前整行不显示，原文不上屏。**排在它后面的会话流条目同时被挂起**（`data-dsh-reveal-hold`）：回答正文、思考正文与译文的揭示队列照常积压、一帧都不提交；后面的工具行（包括不翻译的那些，例如读写文件的行，它们的摘要渲染成文件链接、从来不在翻译范围内）也一并藏起来。所以不会有内容先占住待出现行的位置、等它出现时再被顶下去，也不会有积压在被按住期间悄悄排空、放行时一次性砸出来。队首就绪才轮到它：先把译文挂上、再让整行出现，中文照旧逐字 + 对数淡入打出，屏幕上只会出现译文；随后按阅读顺序依次放行后面的行。命中本地缓存的行即时显示、不扣留；历史回放、已结束的 turn、加载更早历史、插在已知末尾之前的行、以及两条通道都关闭时不扣留。明确失败（降级 / 通道关闭 / 空译文 / 与原文相同）立即放行原文；等待上限 5 秒，超时先显示原文，译文之后到达再原地替换。已显示译文的行若原文被上游改写（如 ask_user 的「已回答 n/m」、失败摘要、todo 完成计数），保留当前译文并在新译文就绪后原地替换，行不消失、不出现英文。
