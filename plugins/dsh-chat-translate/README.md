...
- **实时行整条扣留（按阅读顺序逐行放行）**：当前 turn 里新追加的工具调用行（以及命令行等所有被翻译的行）在标题译文就绪前整条 `display: none`，原文不上屏，也**不占任何位置**——不只是看不见：参与布局的条目会带上会话流的兄弟间距，积压几行就是屏幕底部一条几十像素的空白，所以藏的是整个盒子，等待期间屏幕上是零。**排在它后面的会话流条目整条一起藏**：样式上整条移出布局——不翻译的行（例如读写文件的行，它们摘要渲染成文件链接、从来不在翻译范围内）、宿主直接落盘的正文、已经写入一半的正文，都得等轮到它。所以不会有内容先占住待出现行的位置、等它出现时再被顶下去。队首就绪才轮到它：先把译文挂上、再让整行出现，屏幕上只会出现译文；随后按阅读顺序**一步一行**放出后面的行，一次全放会让整屏同时变出来。解锁时后面可能已经积了一长串行，所以放行步长随队伍长度收缩（总时长约 0.9 秒封顶），既逐行可见也不会让长队拖住屏幕。命中本地缓存的行即时显示、不扣留；历史回放、已结束的 turn、加载更早历史、插在已知末尾之前的行、切到另一个会话时那个会话里已有的内容、以及两条通道都关闭时不扣留。明确失败（降级 / 通道关闭 / 空译文 / 与原文相同）立即放行原文；等待上限 5 秒，超时先显示原文，译文之后到达再原地替换。已显示译文的行若原文被上游改写（如 ask_user 的「已回答 n/m」、失败摘要、todo 完成计数），保留当前译文并在新译文就绪后原地替换，行不消失、不出现英文。
