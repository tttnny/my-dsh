 ...
 *   pkg：rpcCall(endpoint, args)（同源 POST /api/dsws → 传输信封解包 → res.value）
 *
 * 关键约束（实测）：两种方言的调用方看到同一形状 —— 宿主 handler 的原始返回值
 * （{ok, ...} 载荷）。动态 host.call 直返 handler 结果；pkg rpcCall 解包传输信封
 ...
 * 因此这里只用同源 fetch，不持有 connection 服务。宿主侧为何走精确 Fetch 路由、而非
 * connection.rpc.handle，见 src/host/rpcChannel.js 头部注释。
...
export const describe = () => ({
  ...
  pkg: '同源 POST /api/dsws → res.value（统一解包 + 抛错）',
})
