...
// ---------- 构建 client ----------
/** 客户端半边 inject 声明真源 = src/client/index.js 插件对象的 inject 数组；
 *  产物 exports.inject 由此读出，构建脚本不再重复写服务名列表。 */
function extractClientInject(body) {
  const m = /^\s*inject\s*:\s*\[([^\]]*)\]/m.exec(body)
  if (!m) throw new Error('[build] src/client/index.js 插件对象缺 inject 声明（exports.inject 无源）')
  const list = m[1].split(',').map((s) => s.trim().replace(/^['"]|['"]$/g, '')).filter(Boolean)
  if (!list.length) throw new Error('[build] src/client/index.js 的 inject 声明为空')
  return list
}
async function buildClient({ version, repoUrl }) {
  const { header, body } = extractPluginBody('src/client/index.js')
  const clientInject = extractClientInject(body)
...
${PKG_CLIENT_SHIMS}
    ...
    exports.inject = ${JSON.stringify(clientInject)}
