...
const check = function (file) {
  ...
  // 现状 6 槽位：shell.overlay / conversation.input.dock / tool.view.cordis / sidebar.right.pane.tab /
  //   settings.section / sidebar.settings.item。
  // #298 幂等：全部经 __injectOnce 注入，底层 slots.inject 仅剩 helper 内 1 处（变量式调用，不计入字面量 ' 统计）；
  //   此处校验幂等注册数（共享页子槽传的是 SIDEBAR_ITEM_SLOT 常量，故不匹配字面量引号）。
  const nOnce = (src.match(/__injectOnce\s*\(/g) || []).length
  // 调用点计数只看代码：共享页壳的文档注释引用了 `ctx.slots.inject('settings.section', …)` 这句话。
  const codeOnly = src.replace(/\/\*[\s\S]*?\*\//g, '').replace(/^[ \t]*\/\/.*$/gm, '')
  const nRaw = (codeOnly.match(/slots\.inject\('/g) || []).length
