...
check(pc.includes('readSessionPreset') && pc.includes('{ preset:'), 'probe-chain.js 直传客户端解析的 preset')
const ssw = read('src/client/kernel/store-switch.js')
check(ssw.includes('readSessionPreset') && ssw.includes('{ preset:'), 'store-switch.js 确认框链请求同口径直传 preset')
const sss = read('src/client/kernel/store-snapshot.js')
check(sss.includes('readSessionPreset'), 'store-snapshot.js 提供 readSessionPreset（活对象 + 快照行）')
