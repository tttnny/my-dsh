...
// #459：index.js 已拆出装配（panelAssembly.js），router.js 另有右侧边栏标签座位；
// 三处同属一个闭包，幂等口径按「闭包内注入点」合计（意图不变）。
const src = ['src/client/index.js', 'src/client/panelAssembly.js', 'src/client/kernel/router.js'].map((f) => fs.readFileSync(path.resolve(f), 'utf8')).join('\n')
const dev = fs.readFileSync(path.resolve('client.js'), 'utf8')
const pkg = fs.readFileSync(path.resolve('package/lib/client.js'), 'utf8')

// 注入点按源码里的调用写法逐条钉死：共享页子槽经 SIDEBAR_ITEM_SLOT 常量（页壳单源）注册，
// 故该项登记调用片段而不是槽名字面量。
const slots = [
  "__injectOnce('shell.overlay'",
  "__injectOnce('conversation.input.dock'",
  "__injectOnce('tool.view.cordis'",
  "__injectOnce('sidebar.right.pane.tab'",
  "__injectOnce('settings.section'",
  "__injectOnce(SIDEBAR_ITEM_SLOT",
]

// 调用点计数只看代码：页壳的文档注释里引用了 `ctx.slots.inject('settings.section', …)` 这句话，
// 不剥注释会把说明文字算成第二个注入点。
...
slots.forEach(name => {
  check(src.includes(name), 'src ' + name + '）')
})
// 原始直接 slots.inject 应仅剩 helper 内 1 处
const directInjectSrc = (stripComments(src).match(/slots\.inject\s*\(/g) || []).length
...
;[['client.js', dev], ['package/lib/client.js', pkg]].forEach(([label, txt]) => {
  ...
  slots.forEach(name => {
    check(txt.includes(name), label+' ' + name + '）')
  })
  const code = stripComments(txt)
  const cnt = (code.match(/slots\.inject\s*\(/g) || []).length
  check(cnt === 1, label+' slots.inject 仅剩 1 处，实得 '+cnt)
  check(!code.includes("slots.inject('shell.overlay'"), label+' 无裸露 shell.overlay 注入')
  check(!code.includes("slots.inject('sidebar.settings.item'"), label+' 无裸露 sidebar.settings.item 注入')
})
...
// 模拟 __injectOnce 逻辑
{
  ...
  __injectOnceMock(errSlot, () => fakeSlots.register({name:errSlot, id:'dsws-retry'}, {}))
...
console.log('\n-- 边界：异常分支与旧数据 --')
{
  ...
  try {
    ...
    fn('mock-slot', () => fakeSlots2.register())
    fn('mock-slot', () => fakeSlots2.register()) // 第二次应被拦截不抛
  } catch(e){ ok=false }
