...
- **ask_user_grilling**：grilling 轮次专用提问工具。
  - **子代理闸门**：你名下还有 running 的后代子代理（或其状态查不到）时会拒绝提问，返回「阻塞 + 运行中名单」——agent 应**结束当前回合**，等结算通知自动唤醒后再调用，不在同一回合内反复重试；
  ...
  - **描述即纪律**：工具描述保持精简，只承载工具必知项（以 `lib/index.js` 实物为准：轮次专用路由、散文预告 + 同一回合一次投递、字段映射与推荐标记、id 规则、勿自加收尾题、后代子代理闸门的 blocked 现教）；投递协议细节由技能旁注 DSH delivery 承载，不与工具描述重复；停轮纪律不在技能里写，只由本工具描述与 blocked 报错现教。
