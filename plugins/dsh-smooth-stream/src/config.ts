 ...
 * boot global is absent (client-only composition).
 */
...
/**
 * Window global the Host writes into every served index response through the
 * webserver index-injection table. The browser boot graph carries no per-entry
 * config, so this global is the only Host-to-client configuration channel for a
 * composed web plugin.
 */
