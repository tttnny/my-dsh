...
- **设置 › Browser Neo 一键连接**：状态行（端点 / 连接结果 / 最近错误，4s 轮询）+「连接 BrowserOS neo」按钮——Neo 端点无应答时先 `open -g` 后台拉起（macOS，不抢焦点；人的这次点击就是启动授权，模型依旧无权启动）、等待期间持续重读 runtime.json 跟住端口变化、随后重建桥会话（dispose + 重挂）。侧栏 `order: 140`。没有启用开关——关插件直接注释 patch 行。
...
#    中登记 "@lynn123411/dsh-browseros-neo"（版本 1.1.0）后重启 dsh web
```
