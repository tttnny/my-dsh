...
改动只发生在两类文件上：`agent.cordis.yml`（官方正文上加两处 MATT-ADD）与 `skills/grilling/SKILL.md`（本地适配，三份 preset 逐字节相同，改动内容为中文）。persona 一行不改——grilling 纪律不写进 persona，而是下沉到技能旁注与插件工具描述（模型读到技能/工具时正好看到，比 system prompt 里的抽象禁令有效）。
...
上游文件（frontmatter + 三段英文正文）经本地适配后的**完整成品**如下，三份 preset 该文件逐字节相同、可互相覆盖。本地改动：引导行保留上游英文 `Format a round like so:`；模板去 emoji、改为纯文本并把**选项独立成 `Options:` 块**（占位为英文，与上游同风格）；格式块后新增中文「DSH delivery」旁注（**先散文预告本轮问题、再以 `ask_user_grilling` 投递表单**）；删除上游 "Don't block…" 一句；新增中文「Sub-agent rounds」旁注。改动处以 ①–④ 标注在文后；其余正文段落为上游英文原样、非改动。
...
> - 字段映射按上面的模板：`Qn.` 标题 → `header`；正文 → `question`（只写问题本身，选项一律属于 `options`）；`Recommended:` 推荐 → 放 `options` 第一位并加 `"(Recommended)"`；推荐若不在选项中，在正文里简短注明。
> - 散文发出后若本回合没有随之调用 `ask_user_grilling`，立即在同回合补上这一次投递（把**同一轮**原样发出）——散文只预告，作答在表单里完成。
> - 轮次问题不使用普通的 `ask_user_question` 工具。
> - PTC preset 下（模型只见 `run_code`）：把该轮写成 `return await tools.ask_user_grilling({ questions: [...] })` 放在 `run_code({ code, description })` 程序内，程序自身也要带顶层的 `code` 与 `description`；散文预告照常写在消息文本里，映射规则相同。
...
> 一旦你在某轮派遣了子代理，先输出每个子代理的任务清单（各自要去查什么），然后**停**：不再调用任何其他工具、不提问、结束本回合。子代理的结算通知会自动唤醒你——不要轮询。等**全部**已派遣子代理结算后，再问 frontier（包括未受阻的问题）。后台有子代理运行时 `ask_user_grilling` 会硬拒绝（返回 `blocked: true`）——若被拒就结束回合等待，绝不在同一回合内重试。   # ④
...
- **②** 在格式块后新增旁注「**DSH delivery**」整段（中文）：轮次投递 = **先在消息文本里按模板以散文预告本轮全部问题（标题/正文/选项），再对同一轮调用一次 `ask_user_grilling` 投递表单**；预告与投递必须同一轮、一一对应；字段映射（`Qn.` 标题→`header`、正文→`question`、选项→`options`、`Recommended:` 推荐→置首并加 "(Recommended)"）；散文发出后必须同回合补上工具投递（散文只预告、作答在表单）；PTC 下同样先散文预告、再经 `run_code` 内 `tools.ask_user_grilling` 投递。**原因**：纯散文让用户拿到不可点击文本、丢失表单；纯工具又让用户看不到正文里的问题陈述——散文预告 + 工具表单各司其职，且必须成对出现。
...
- **Matt 技能上游更新后**：整体覆盖 25 个技能目录，再以本仓库任一 `grilling/SKILL.md` 成品覆盖回 grilling（按第二节重打 4 处适配，其余技能无本地改动）。
