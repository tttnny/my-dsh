...
| [matt-standard](./presets/matt-standard) | **Matt 标准工程模式**：官方 `standard` 逐字（persona 零改动）+ Matt Pocock 25 个工程/生产力技能（[mattpocock/skills](https://github.com/mattpocock/skills)）+ grilling 适配插件；grilling 纪律下沉到 `skills/grilling/SKILL.md` 本地旁注与插件工具描述，组合仅两处 MATT-ADD 附加改动 |
| [matt-ptc](./presets/matt-ptc) | **Matt PTC 模式（实验性）**：官方 `ptc` 逐字（`mode: ptc`，模型只见 `run_code`）+ 25 个 Matt 技能 + grilling 适配插件（grilling 交互折叠进 SDK 脚本，技能旁注含 PTC 措辞） |
| [matt-cordis](./presets/matt-cordis) | **Matt 创造模式**：官方 `cordis` 逐字（`tool-cordis` 动态插件工具集、两个随附技能、双平面 persona）+ 25 个 Matt 技能并入 skills/ + grilling 适配插件，组合仅一处 MATT-ADD 附加改动 |
...
| [matt-presets-bootstrap](./patches/matt-presets-bootstrap/) | **新机器构建三个 matt preset 的指导与脚本**：原厂 preset 逐字 + Matt 25 技能 + grilling 适配插件的组装原理（MATT-ADD 附加改动）、一键 setup（插件同步 + preset 同步 + 自检）、grilling 会话 check 验收与「散文轮失守」排错框架。详见 [README](./patches/matt-presets-bootstrap/README.md) |
