...
| [@lynn123411/dsh-agent-workflow](./plugins/dsh-agent-workflow) | `dsh.bundle` + `dsh.client/web` | **Agent 工作流**：独立「工作流」标签页，按用户对话轮次呈现模型请求 / 响应 / 工具调用执行链路（完整 request messages / system / tools 检查、Token 与缓存统计、运行中/完成/失败状态、加载更早回合），适配 dsh 0.1.2-alpha.1 | `dsh plugin --profile web add @lynn123411/dsh-agent-workflow` |
