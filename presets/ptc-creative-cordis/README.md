...
安装后与官方 `standard` / `code` / `cordis` 预设并列，可在新建会话时直接选用。

## 安装与启用

```bash
# 1. 创建 preset 目录（preset id 必须为 ptc-creative-cordis）
mkdir -p ~/.dsh/.agent-presets/ptc-creative-cordis

# 2. 复制 preset 配置文件与技能目录（无需复制 README.md）
cp ptc-creative-cordis/agent.cordis.yml ptc-creative-cordis/preset.yml ~/.dsh/.agent-presets/ptc-creative-cordis/
cp -R ptc-creative-cordis/skills ~/.dsh/.agent-presets/ptc-creative-cordis/
```

重启 DSH 后，在新建会话界面选择「PTC-创造 混合模式」（或在 Settings -> General 设为默认）。

## 目录结构

```
ptc-creative-cordis/
├── agent.cordis.yml       # Preset 主配置（基于 code 模式叠加 cordis 创造能力）
...
### 一键修复
对全局 `dsh-tool-cordis` 的 `lib/index.js` 添加幂等判断（同 ID provider 跳过重复注册）：

```bash
node -e '
const fs = require("fs"), cp = require("child_process");
const root = cp.execSync("npm root -g 2>/dev/null || pnpm root -g 2>/dev/null").toString().trim();
const target = cp.execSync(`find "${root}" -type f -path "*dsh-tool-cordis/lib/index.js" 2>/dev/null | head -n 1`).toString().trim();
